...
exports.getAllCustomer = async (req, res) => {
  try {
    const customerList = await customerModel.getCustomers();
    const totalItems = customerList.length;
    const allCustomerList = customerList.map((customer, index) => ({
      ...customer,
      custom_id: index + 1,
    }));
    res.status(200).json({
      message: "All Customer List",
      totalCustomer: totalItems,
...
exports.getCustomerById = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: `Error fetching Customer: ${error.message}` });
  }
};

// PUT update by id
exports.postUpdateById = [
  middleware.single("file"),
  async (req, res) => {
    const id = req.params.id;
    const data = {
